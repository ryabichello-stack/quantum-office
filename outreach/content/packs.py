...
- помогаем согласовать условия и подключение + tech + сопровождение;
- продающий текст с хуками и **жирными** акцентами.

Ломбарды (холодная цепочка):
- не «замена наличным» и не «массовые выплаты» как герой-фрейм;
- опциональный канал: карта / СБП рядом с привычной выдачей наличными;
- «менять существующую схему не нужно»;
- social proof: официальные партнёры банков из топ-5 РФ (не «~105 банков»);
- CTA: «Узнать условия», а не «Перезвонить».
"""
...
PACKS: dict[str, dict[str, Any]] = {
    "lombards": {
        ...
        "title": "Ломбарды",
        "short": (
            "Карта и СБП рядом с наличными: клиент выбирает сам; "
        ...
        "steps": [
            _step(
                ...
                delay_days=0,
                label="intro",
                subject="Выдавайте займы клиентам сразу на карту или по СБП",
            ...
            _step(
                ...
                delay_days=3,
                label="scenarios",
                subject="Как клиент получает деньги на карту или по СБП — без смены вашей схемы",
                plain=(
                    "{greeting}\n\n"
                    "**Как это выглядит на практике**\n\n"
                    "Речь не о замене наличных, а о ещё одном способе выдачи рядом "
                    "с привычным.\n\n"
                    "Типовой путь:\n\n"
                    "- Сделка оформлена\n"
                    "- Клиент выбирает: наличные, карта или СБП\n"
                    "- При безнале деньги уходят практически сразу на карту или по СБП\n"
                    "- Статус возвращается в вашу систему для контроля и сверки\n\n"
                    "Работать можно через личный кабинет, реестр или вашу учётную систему — "
                    "подключение и согласование условий берём на себя.\n\n"
                    "Деньги идут напрямую через банк, не проходя через Quantum Payouts. "
            ...
            _step(
                ...
                delay_days=6,
                label="economics",
                subject="Какие условия доступны под ваш объём — без обязательств что-то менять",
                plain=(
                    "{greeting}\n\n"
                    "**Сначала условия — потом решения**\n\n"
                    "Мы не предлагаем отказаться от наличных и не спорим с вашей "
                    "текущей моделью. Задача проще: понять, какие условия по карте "
                    "и СБП доступны именно под ваш объём.\n\n"
                    "Что обычно уточняем:\n\n"
                    "- ориентировочный объём выплат в месяц\n"
                    "- есть ли уже безналичная выдача или пока только наличные\n"
                    "- удобнее личный кабинет, реестр или связка с учётом\n\n"
                    "Комиссия зависит от банка и объёма — ориентировочно от 1,5% до 0,4%. "
                    "Подключение и согласование условий — на нашей стороне.\n\n"
                    "Точные цифры не нужны: хватит примерного ориентира или ответа "
                    "«хотим узнать условия».\n\n"
                    "{signature}"
                ),
            ),
            _step(
                ...
                delay_days=10,
                label="broader",
                subject="Если сейчас не до этого — оставлю тему открытой",
                plain=(
                    "{greeting}\n\n"
                    "Если **выдача на карту / СБП** сейчас не в приоритете — "
                    "это нормально. Менять схему «насильно» мы не предлагаем.\n\n"
                    "Когда понадобится просто добавить безналичный способ рядом "
                    "с наличными — Quantum Labs рядом: подключение, согласование "
                    "условий, работа через кабинет, реестр или вашу систему.\n\n"
                    "Если актуально шире выплат клиентам — также помогаем "
                    "с приёмом платежей, эквайрингом и API-слоем. Без давления: "
                    "можно вернуться к теме, когда будет удобно.\n\n"
                    "{signature}"
                ),
            ),
            _step(
                ...
                delay_days=15,
                label="close",
                subject="На связи, если понадобится выдача на карту или по СБП",
                plain=(
                    "{greeting}\n\n"
                    "Закрываю переписку, чтобы **не быть навязчивым**.\n\n"
                    "Если позже захотите добавить клиентам выбор — наличные, карта "
                    "или СБП — мы на связи: официальные партнёры банков из топ-5, "
                    "подключение на нашей стороне, условия под ваш объём.\n\n"
