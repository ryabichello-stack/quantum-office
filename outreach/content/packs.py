...
- ясно: подключаем услугу **массовых выплат** (карты / СБП);
- даже если выплаты уже есть — можно согласовать более выгодные условия
  (ориентир комиссии примерно 1,5% … 0,4% при больших объёмах);
- пул банков (~105): условия в текущем банке или у партнёра;
- tech: интеграция в систему клиента или личный кабинет;
- технологический партнёр, не посредник; договор с банком — у клиента.
...
PACKS: dict[str, dict[str, Any]] = {
    "lombards": {
        "id": "lombards",
        "title": "Ломбарды",
        "short": "Массовые выплаты клиентам (карты/СБП): подключение, ставка ориентир 1,5%…0,4%, ~105 банков, интеграция или ЛК",
        ...
        "steps": [
            _step(
                ...
                delay_days=0,
                label="intro",
                subject="Подключение массовых выплат клиентам — карты и СБП",
            ...
            _step(
                ...
                delay_days=3,
                label="scenarios",
                subject="Как выглядит массовая выплата клиенту на практике",
                plain=(
                    "{greeting}\n\n"
                    "Продолжу по сути — **как выглядит услуга массовых выплат** для ломбарда:\n\n"
                    "- выплата клиенту **на карту** или **по СБП**;\n"
                    "- массовые операции **по реестру**;\n"
                    "- выплата **из вашей учётной системы** или через **личный кабинет**;\n"
                    "- статусы операций и сверка по сети.\n\n"
                    "Типовой путь:\n\n"
                    "**сделка → способ получения → выплата → деньги клиенту → статус обратно в учёт.**\n\n"
                    "Банк — **ваш прямой договор** (текущий или партнёр из пула ~105). "
                    "Мы помогаем **подключить** контур и **согласовать экономику** "
                    "под объём (ориентир комиссии **1,5% … 0,4%**).\n\n"
                    "Напишите **примерно объём выплат в месяц** — вернёмся с вариантами по делу.\n\n"
                    "{signature}"
                ),
            ),
            _step(
                ...
                delay_days=6,
                label="economics",
                subject="Сравним условия выплат — в вашем банке или у партнёра",
                plain=(
                    "{greeting}\n\n"
                    "Самый честный вход: **не ломать работающую схему**, а сначала **сравнить условия**.\n\n"
                    "Даже если массовые выплаты уже есть — часто можно согласовать "
                    "**более выгодную комиссию**. Ориентир: примерно **от 1,5% до 0,4%** "
                    "при больших объёмах (не прайс-лист, а вилка для разговора).\n\n"
                    "Работаем примерно со **105 банками**: можно улучшить условия "
                    "**в вашем банке** или рассмотреть **другой банк-партнёр**.\n\n"
                    "Что сравниваем:\n"
                    "- текущий способ выплат;\n"
                    "- объём и число операций;\n"
                    "- стоимость схемы;\n"
                    "- нужна ли интеграция или достаточно личного кабинета.\n\n"
            ...
            _step(
                ...
                delay_days=10,
                label="broader",
                subject="Если массовые выплаты «не сейчас»: чем ещё полезен Quantum Labs",
                plain=(
                    "{greeting}\n\n"
                    "Если **массовые выплаты** сейчас не в приоритете — оставлю коротко шире.\n\n"
                    "Quantum Labs — технологический партнёр по платёжному контуру: "
                    "подключение к банкам, сопровождение, интеграция.\n\n"
                    "Помимо выплат клиентам также:\n"
                    "- приём платежей и **эквайринг**;\n"
                    "- API и автоматизация сверки;\n"
                    "- платёжный слой под ваш продукт.\n\n"
                    "Снова: **прямой договор с банком**, мы — tech-партнёр "
                    "по подключению, условиям и сопровождению (~105 банков в пуле).\n\n"
                    "Есть задача, которую банк/провайдер решает **дорого или вручную**? "
                    "Опишите одним абзацем — предложим вариант.\n\n"
                    "{signature}"
                ),
            ),
            _step(
                ...
                delay_days=15,
                label="close",
                subject="Закрываю тему — остаюсь на связи по массовым выплатам",
                plain=(
                    "{greeting}\n\n"
                    "Закрываю переписку, чтобы **не быть навязчивым**.\n\n"
                    "Когда понадобится **подключить массовые выплаты** клиентам "
                    "(карты / СБП), согласовать условия или сравнить комиссию — "
                    "Quantum Labs рядом: ~105 банков, интеграция или личный кабинет, "
                    "ориентир ставки **1,5% … 0,4%** под объём.\n\n"
