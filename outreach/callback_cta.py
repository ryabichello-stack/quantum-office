...
_DEFAULT_TITLE = "Хотите узнать условия под ваш объём?"
_DEFAULT_BUTTON = "Узнать условия"
_DEFAULT_GREETING = (
    "Здравствуйте! Это Quantum Labs — вы оставили заявку узнать условия с письма."
)
_DEFAULT_SCRIPT = (
    "Ты — голосовой ассистент Quantum Labs. Клиент оставил заявку «узнать условия» "
    "из email-рассылки. Представься коротко, уточни удобное время и тему разговора, "
    "кратко расскажи, что можно добавить выдачу на карту или по СБП рядом с наличными "
    "без смены существующей схемы, и предложи следующий шаг "
    "(созвон с менеджером / подбор условий). Будь вежлив и конкретен."
)
_DEFAULT_LEAD = (
    "Уточним несколько параметров и предложим условия под ваш объём."
)
...
def form_page_html(
    *,
    ...
    if done:
        thanks = escape(
            done_message
            or "Спасибо! Заявка принята — уточним параметры и вернёмся с условиями."
        )
        body = (
            f"<h1>{title}</h1>"
        ...
        <p class="note">Уточним несколько параметров и предложим условия под ваш объём.</p>
        """
