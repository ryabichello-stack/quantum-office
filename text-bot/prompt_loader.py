...
- В ПЕРВОМ ответе сразу покажи полный сценарий: номер + Greeting + Script
  (вызови draft_outbound_call и вставь результат в сообщение).
- ЗАПРЕЩЕНО писать «подтвердите черновик» / «на связи» БЕЗ текста greeting и script
  в том же сообщении.
