...
KNOWLEDGE_HEALTH_URL = os.getenv("KNOWLEDGE_HEALTH_URL", "http://127.0.0.1:8017/health")
CALENDAR_HEALTH_URL = os.getenv("CALENDAR_HEALTH_URL", "http://127.0.0.1:8014/health")
CONFERENCE_HEALTH_URL = os.getenv("CONFERENCE_HEALTH_URL", "http://127.0.0.1:8016/health")
FILES_HEALTH_URL = os.getenv("FILES_HEALTH_URL", "http://127.0.0.1:8015/health")
...
# AstDB family/key for inbound auto-answer line (dialplan reads DB(quantum/inbound_line))
INBOUND_LINE_FAMILY = os.getenv("INBOUND_LINE_FAMILY", "quantum").strip() or "quantum"
INBOUND_LINE_KEY = os.getenv("INBOUND_LINE_KEY", "inbound_line").strip() or "inbound_line"
...
def _inbound_line_raw() -> str | None:
    """AstDB value for inbound line, or None if missing/unreadable."""
    rc, out = _run(
        ["asterisk", "-rx", f"database get {INBOUND_LINE_FAMILY} {INBOUND_LINE_KEY}"],
        timeout=5,
    )
    if rc != 0:
        return None
    ...
    for line in text.splitlines():
        if ":" in line and "value" in line.lower():
            return line.split(":", 1)[1].strip().lower()
    # Some Asterisk builds print just the value
    if text and " " not in text and len(text) < 32:
        return text.lower()
    return None


def _inbound_line_state() -> dict[str, Any]:
    """Inbound auto line: missing key defaults to ON (accept calls)."""
    raw = _inbound_line_raw()
    if raw is None:
        enabled = True
        source = "default"
        value = "on"
    ...
    return {
        ...
        "raw": raw,
        "source": source,
        "family": INBOUND_LINE_FAMILY,
        "key": INBOUND_LINE_KEY,
        "astdb_path": f"{INBOUND_LINE_FAMILY}/{INBOUND_LINE_KEY}",
        "label": "Автолиния входящих",
        "hint": "Когда выкл — Asterisk не пускает звонок в ИИ-секретаря",
    }


def _inbound_line_set(enabled: bool) -> dict[str, Any]:
    value = "on" if enabled else "off"
    rc, out = _run(
        [
            "asterisk",
            "-rx",
            f"database put {INBOUND_LINE_FAMILY} {INBOUND_LINE_KEY} {value}",
        ],
        timeout=5,
    )
    if rc != 0:
        raise HTTPException(500, f"failed to set inbound line: {out[:400]}")
    state = _inbound_line_state()
    state["set_output"] = (out or "")[:200]
    return state


def _recent_calls_glance(limit: int = 8) -> list[dict[str, Any]]:
    ...
    try:
        ...
        logger.warning("recent calls glance failed: %s", exc)
        return []
    finally:
        conn.close()


def _outreach_glance() -> dict[str, Any] | None:
    ...
    return {
        ...
        "run_state": data.get("run_state") or "stopped",
        "daily_limit": data.get("daily_limit"),
        "effective_daily_limit": data.get("effective_daily_limit"),
...
def _campaign_glance() -> dict[str, Any] | None:
    headers: dict[str, str] = {}
    if CAMPAIGN_TOKEN:
        headers["Authorization"] = f"Bearer {CAMPAIGN_TOKEN}"
        headers["X-Webhook-Token"] = CAMPAIGN_TOKEN
    data = _http_json(f"{CAMPAIGN_BASE}/api/status", headers=headers, timeout=4.0)
    if not data:
        data = _http_json(CAMPAIGN_HEALTH_URL, headers=headers, timeout=3.0)
    if not data:
        return None
    ...
    return {
        ...
        "processed": camp.get("processed"),
        "interested": camp.get("interested"),
        "errors": camp.get("errors"),
        "queued": camp.get("queued"),
        "message": camp.get("message"),
        "started_at": camp.get("started_at"),
        "finished_at": camp.get("finished_at"),
        "last": camp.get("last") if isinstance(camp.get("last"), dict) else None,
    }
...
def api_channels_report(
    from_day: str | None = None,
    to_day: str | None = None,
    days: int = 30,
    x_console_token: str | None = Header(default=None),
) -> dict[str, Any]:
    """Unified engagement report: messengers, email, calls, Tilda."""
    _require_token(x_console_token)
    try:
        return channels_report.build_channels_report(
            from_day=from_day,
            to_day=to_day,
            default_days=max(1, min(int(days or 30), 366)),
        )
    except ValueError as exc:
        raise HTTPException(400, str(exc)) from exc


@app.post("/api/channels/tilda/lead")
async def api_tilda_lead(request: Request) -> dict[str, Any]:
    """Public webhook for Tilda forms (secret via query or header).

    In Tilda: Site settings → Forms → Webhook URL:
      https://a.47z.ru/_quantum_console/api/channels/tilda/lead?token=YOUR_SECRET
    """
    expected = (
    ...
    got = (
        ...
        or (request.headers.get("X-Tilda-Token") or "").strip()
        or (request.headers.get("X-Webhook-Token") or "").strip()
    )
    if not expected or got != expected:
        raise HTTPException(401, "bad tilda webhook token")
...
def api_status(x_console_token: str | None = Header(default=None)) -> dict[str, Any]:
    ...
    # Confirm dialplan gates inbound line via AstDB
    rc_in, in_out = _run(["asterisk", "-rx", "dialplan show from-mango"], timeout=10)
    inbound_line_gated = "inbound_line" in in_out or "quantum/inbound_line" in in_out
    ...
    services = [
        {
            ...
            "ok": health["ai_engine"],
            "hint": "Realtime голос · docker ai_engine · робот на линии",
            "group": "voice",
        },
        {
            "id": "text_bot",
        ...
        },
        {
            ...
            "ok": health["outreach"],
            "hint": "порт 8012 · Bitrix / SMTP outreach",
            "group": "outreach",
        },
        {
            "id": "knowledge",
            "label": _SERVICE_LABELS["knowledge"],
            "ok": health["knowledge"],
            "hint": "порт 8017 · общая база знаний",
            "group": "office",
        },
        {
            "id": "calendar",
            "label": _SERVICE_LABELS["calendar"],
            "ok": health["calendar"],
            "hint": "порт 8014 · CalDAV слоты и события",
            "group": "office",
        },
        {
            "id": "conference",
            "label": _SERVICE_LABELS["conference"],
            "ok": health["conference"],
            "hint": "порт 8016 · Телемост + приглашения",
            "group": "office",
        },
        {
            "id": "files",
            "label": _SERVICE_LABELS["files"],
            "ok": health["files"],
            "hint": "порт 8015 · брокер файлов",
            "group": "office",
        },
        {
            "id": "mailer",
        ...
        {
            "id": "inbound_line_gate",
            "label": "Гейт автолинии",
            "ok": inbound_line_gated,
            "hint": "dialplan from-mango читает AstDB quantum/inbound_line",
            "group": "voice",
        },
    ]
    units_ui = [
    ...
    return {
        "host": host_raw,
        "host_label": "Quantum Labs · пульт секретаря",
        ...
        "inbound_line_gated": inbound_line_gated,
        "robot": {
            "ok": robot_ok,
            "label": "Робот на приёме",
            "ai_engine": health["ai_engine"],
            "sip": mango_registered,
            "line_enabled": inbound_line["enabled"],
            "hint": "ИИ + SIP + автолиния включены",
        },
        "activity": {
            "recent_calls": _recent_calls_glance(8),
            "outreach": _outreach_glance(),
            "campaign": _campaign_glance(),
        },
        "outbound": {
...
class InboundLineBody(BaseModel):
    enabled: bool = Field(..., description="True = принимать входящие на ИИ-секретаря")


@app.get("/api/line")
def api_line_get(x_console_token: str | None = Header(default=None)) -> dict[str, Any]:
    _require_token(x_console_token)
    return {"ok": True, "inbound_line": _inbound_line_state()}


@app.post("/api/line")
def api_line_set(
    body: InboundLineBody,
    x_console_token: str | None = Header(default=None),
) -> dict[str, Any]:
...
_UNIT_LABELS = {
    ...
    "ava-knowledge": "База знаний",
    "ava-calendar": "Календарь",
    "ava-conference": "Телемост",
    "ava-files": "Файлы",
