(() => {
  ...
  const TAB_META = {
    ...
    channels: {
      title: "Каналы",
      hint: "Telegram, Max, email, Tilda, входящие и исходящие звонки за период",
    },
  ...
  function setTab(name) {
    ...
    if (name === "channels") {
      initChannelsDates();
      loadChannelsReport().catch((e) => {
        if ($("channelsBox")) $("channelsBox").textContent = String(e.message || e);
      });
    }
    if (name === "outreach") {
  ...
  function initChannelsDates() {
    if (!$("chFrom") || !$("chTo")) return;
    if ($("chFrom").value && $("chTo").value) return;
    const to = new Date();
    const from = new Date();
    from.setDate(to.getDate() - 29);
    $("chFrom").value = isoDay(from);
    $("chTo").value = isoDay(to);
  }

  function applyChannelsPreset(days) {
    const to = new Date();
    const from = new Date();
    from.setDate(to.getDate() - (Math.max(1, Number(days) || 30) - 1));
    if ($("chFrom")) $("chFrom").value = isoDay(from);
    if ($("chTo")) $("chTo").value = isoDay(to);
  }
  ...
  async function loadChannelsReport() {
    ...
    const to = ($("chTo") && $("chTo").value) || "";
    const box = $("channelsBox");
    if (!box) return;
    box.innerHTML = `<div class="surface">Загрузка отчёта…</div>`;
    const q = new URLSearchParams();
    if (from) q.set("from_day", from);
    if (to) q.set("to_day", to);
    const r = await api("/api/channels/report?" + q.toString());
    const s = r.summary || {};
    const email = r.email || {};
    const calls = r.calls || {};
    const tilda = r.tilda || {};
    ...
    box.innerHTML = `
      ...
      period.to
    )}</strong> · ${esc(period.days)} дн. · обновлено ${esc(fmtMsk(r.generated_at) || r.generated_at || "")}</p>

      <div class="kpi-grid">
        ${kpiCard("Telegram · чаты", s.telegram_chats ?? 0, (s.telegram_messages ?? 0) + " сообщ. от людей")}
      ...
      <div class="channels-columns">
        <div class="surface">
          <h2 class="section-title">Мессенджеры</h2>
          <table class="calls-table"><thead><tr><th>Канал</th><th>Чаты</th><th>Входящие сообщ.</th><th>Ответы бота</th></tr></thead>
          <tbody>
            <tr><td>Telegram (+ Business)</td><td>${esc(tg.new_chats ?? 0)}</td><td>${esc(
      tg.user_messages ?? 0
    )}</td><td>${esc(tg.assistant_messages ?? 0)}</td></tr>
            <tr><td>Max</td><td>${esc(mx.new_chats ?? 0)}</td><td>${esc(
      mx.user_messages ?? 0
    )}</td><td>${esc(mx.assistant_messages ?? 0)}</td></tr>
          </tbody></table>
        </div>
        <div class="surface">
          <h2 class="section-title">Email-рассылка</h2>
          <table class="calls-table"><tbody>
            <tr><td>Отправлено</td><td>${esc(email.sent ?? 0)}</td></tr>
            <tr><td>Доставлено (оценка)</td><td>${esc(email.delivered ?? 0)}</td></tr>
            <tr><td>Открыто</td><td>${esc(email.opened ?? 0)} (${esc(emailOpenPct)})</td></tr>
            <tr><td>Ответы</td><td>${esc(email.replied ?? 0)}</td></tr>
            <tr><td>Bounce</td><td>${esc(email.bounced ?? 0)}</td></tr>
            <tr><td>Отписки</td><td>${esc(email.unsubscribed ?? 0)}</td></tr>
            <tr><td>Очередь pending</td><td>${esc(email.pending_queue ?? "—")}</td></tr>
          </tbody></table>
          <p class="muted tight">${esc((email.notes && email.notes.opened) || "")}</p>
        </div>
        <div class="surface">
          <h2 class="section-title">Звонки</h2>
          <table class="calls-table"><tbody>
            <tr><td>Входящие</td><td>${esc((calls.inbound || {}).count ?? 0)} · completed ${esc(
      (calls.inbound || {}).completed ?? 0
    )}</td></tr>
            <tr><td>Исходящие</td><td>${esc((calls.outbound || {}).count ?? 0)} · completed ${esc(
      (calls.outbound || {}).completed ?? 0
    )}</td></tr>
            <tr><td>Прочее / демо</td><td>${esc((calls.other || {}).count ?? 0)}</td></tr>
            <tr><td>Всего</td><td>${esc(calls.total ?? 0)}</td></tr>
          </tbody></table>
        </div>
        <div class="surface">
          <h2 class="section-title">Tilda / сайт</h2>
          <table class="calls-table"><tbody>
          ...
          <p class="muted tight">${esc((tilda.notes && tilda.notes.leads) || "")}</p>
          ${
            (tilda.recent || []).length
  ...
  if ($("btnChannelsLoad")) {
    $("btnChannelsLoad").addEventListener("click", () => {
      loadChannelsReport().catch((e) => {
        if ($("channelsBox")) $("channelsBox").textContent = String(e.message || e);
      });
    });
  }
  document.querySelectorAll("[data-ch-preset]").forEach((btn) => {
    btn.addEventListener("click", () => {
      applyChannelsPreset(btn.getAttribute("data-ch-preset"));
      loadChannelsReport().catch((e) => {
        if ($("channelsBox")) $("channelsBox").textContent = String(e.message || e);
      });
    });
