...
| # | Вопрос | Рекомендация |
|---|--------|--------------|
| R1 | SoT компаний: Bitrix или local Account? | Local Account + Bitrix sync adapter |
| R2 | Где хранить Account DB? | `outreach/data` или новый `platform/` SQLite→PG |
| R3 | LLM для inbox classification сейчас? | Сначала rules; LLM optional feature flag |
| R4 | Порядок: Slice A до или после merge #10? | **После merge** |
| R5 | Отдельный сервис `social-intel`? | Нет до нагрузки; модуль в outreach/console monorepo |
| R6 | ADR Second Brain Accept? | Переиспользовать brain_platform без rewrite |
